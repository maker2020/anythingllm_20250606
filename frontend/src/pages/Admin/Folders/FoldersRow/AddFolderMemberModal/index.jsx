...
export default function AddFolderMemberModal({ closeModal, folder, users }) {
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedUsers, setSelectedUsers] = useState(folder?.userIds || []);

  const handleUpdate = async (e) => {
    e.preventDefault();
    const { success, error } = await Admin.updateUsersInFolder(
      folder.id,
      selectedUsers
    );
    if (success) {
